...
from src.message import message_send_v1, message_remove_v1, message_senddm_v1, message_react_v1
...
def test_messages_react_invalid_token(fix):
    with pytest.raises(AccessError):
        message_react_v1("abc", fix["channel_message_id_1"], 1)


def test_messages_react_invalid_message_id(fix):
    with pytest.raises(InputError):
        message_react_v1(fix["user_1"]["token"], -1, 1)
...
def test_messages_react_success_and_already_reacted(fix):
    ...
    assert u_id_2 in dm_mess_dic["reacts"][0]["u_ids"]
    assert u_id_1 in channel_mess_dic["reacts"][0]["u_ids"]
    with pytest.raises(InputError):
        message_react_v1(fix["user_1"]["token"],
                         fix["channel_message_id_1"], 1)
    with pytest.raises(InputError):
    ...
    channel_mess_dic = help.find_message_in_channel(
        fix["channel_message_id_1"])
    dm_mess_dic = help.find_message_in_dm(fix["dm_message_id_1"])
    assert dm_mess_dic["reacts"][0]["is_this_user_reacted"] == True
    assert channel_mess_dic["reacts"][0]["is_this_user_reacted"] == True
