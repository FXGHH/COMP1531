...
from src.message import message_send_v1, message_remove_v1, message_senddm_v1, message_share_v1, message_pin_v1
...
def test_messages_pin_invalid_token(fix):
    with pytest.raises(AccessError):
        message_pin_v1("abc", fix["channel_message_id_1"])


def test_messages_pin_invalid_message_id(fix):
    with pytest.raises(InputError):
        message_pin_v1(fix["user_1"]["token"], 101)


def test_messages_pin_user_unpermission_in_channle_1(fix):
    with pytest.raises(AccessError):
        message_pin_v1(fix["user_2"]["token"], fix["channel_message_id_1"])


# def test_messages_pin_user_unpermission_in_channle_2(fix):
#     channel_invite_v2(fix["user_1"]["token"],
#                       fix["channel_message_id_1"], fix["user_4"]['auth_user_id'])
#     with pytest.raises(AccessError):
#         message_pin_v1(fix["user_4"]["token"], fix["channel_message_id_1"])


def test_messages_pin_user_unpermission_in_dm_1(fix):
    with pytest.raises(AccessError):
        message_pin_v1(fix["user_1"]["token"], fix["dm_message_id_1"])


def test_messages_pin_user_unpermission_in_dm_2(fix):
    with pytest.raises(AccessError):
        message_pin_v1(fix["user_3"]["token"], fix["dm_message_id_1"])


def test_messages_pin_success_in_channle(fix):
    ...
    assert message_detail['messages'][0]["is_pinned"] == True


def test_messages_pin_success_in_dm(fix):
...
def test_messages_pin_already_pinned_in_channle(fix):
    message_pin_v1(fix["user_1"]["token"], fix["channel_message_id_1"])
    with pytest.raises(InputError):
        message_pin_v1(fix["user_1"]["token"], fix["channel_message_id_1"])


def test_messages_pin_already_pinned_in_dm(fix):
    message_pin_v1(fix["user_2"]["token"], fix["dm_message_id_1"])
    with pytest.raises(InputError):
        message_pin_v1(fix["user_2"]["token"], fix["dm_message_id_1"])
